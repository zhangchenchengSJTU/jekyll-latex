...
\begin{corollary}
	...
	\begin{proof}
		($\to$) 方向由态射唯一嵌入好三角 (\Cref{prop:triangulated-kernel-cokernel}) 与 TR2 即得. 下证明 ($\gets$) 方向. 若所有态射都是 inflation, 则 $X \to 0$ 均是 inflation. 若 $\mathcal{C}$ 有内射对象, 则内射对象不为非零对象; 另一方面, 容易检验零对象确实是内射对象. 对偶地, $\mathcal{C}$ 的投射对象恰好是零对象. 这说明 $\mathcal{C}$ 是 Frobenius 范畴. 由\Cref{thm:extri-frobenius-happel}, $\mathcal{C} / 0 = \mathcal{C}$ 的骨架是三角范畴, 从而 $\mathcal{C}$ 是三角范畴.
	\end{proof}
\end{corollary}