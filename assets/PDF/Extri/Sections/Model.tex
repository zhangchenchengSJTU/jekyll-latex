...
\begin{lemma}\label{lem:cotorsion-pair-criterion}
	给定 $\mathcal{U} \perp \mathcal{V}$, 并假定 $\mathcal{U}$ 与 $\mathcal{V}$ 对直和项封闭. 此时有
	\begin{equation}
		\mathrm{Cone}(\mathcal{V}, \mathcal{U}) = \mathcal{C} \implies \mathcal{U} = {}^\perp \mathcal{V},
	\end{equation}
	对偶地有
	\begin{equation}
		\mathrm{coCone}(\mathcal{V}, \mathcal{U}) = \mathcal{C} \implies \mathcal{V} = \mathcal{U}^\perp.
	\end{equation}
	\begin{proof}
...
\begin{remark}
	特别地, \Cref{lem:cotorsion-pair-criterion} 关于直和项封闭的假定不可缺少. 例如, 取 $(\mathcal{U}, \mathcal{V})$ 是模范畴的余挠对. 给定对象类 $\mathcal{X}$ 与基数 $\kappa$, 定义 $\mathcal{X}^\kappa = \{\prod_\kappa X \mid X \in \mathcal{X}\}$. 由直积是正合函子, 得 $\mathcal{U}^\kappa \perp \mathcal{V}^\kappa$. 假定 $\mathcal{U}$ 与 $\mathcal{V}$ 中存在基数不超过 $\kappa_0$ 的非零对象, 则对任意无限基数 $\kappa > \kappa _0$, 总有真包含 $\mathcal{U}^\kappa \subset \mathcal{U}$ 与 $\mathcal{V}^\kappa \subset \mathcal{V}$. 依照 Eilenburg 位移技巧 (如 \cite{bassBigProjectiveModules1963}), 垂直对 $(\mathcal{U}^\kappa, \mathcal{V}^\kappa)$ 满足\Cref{lem:cotorsion-pair-criterion} 的题设, 但不构成余挠对.
\end{remark}
...
\subsection{Hovey 孪生余挠对 \texorpdfstring{$\to$}{} 相容闭模型结构}
