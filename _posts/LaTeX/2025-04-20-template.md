...
{% ex %}
在特殊情况下, 可直接使用 `tikz` 环境插入 [`quiver`](https://q.uiver.app/) 代码 (小 bug, 需要将 `textcolor` 替换作 `color`). 可以使用行内公式 {% tikzinline %} % https://q.uiver.app/#q=WzAsMyxbMCwxLCJcXGJ1bGxldCJdLFsxLDAsIlxcYnVsbGV0Il0sWzEsMSwiXFxidWxsZXQiXSxbMCwxXSxbMSwyXSxbMyw0LCIiLDIseyJzaG9ydGVuIjp7InNvdXJjZSI6MjAsInRhcmdldCI6MjB9fV1d
\begin{tikzcd}
	& \bullet \\
	\bullet & \bullet
