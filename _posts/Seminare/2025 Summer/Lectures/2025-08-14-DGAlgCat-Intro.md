...
- The term functor category indicates that $𝐅𝐮𝐧 (\mathcal{C},\mathcal{D})$ is an object in $𝐂𝐚𝐭$
...
{% def %}
(DG Category $𝐃𝐆𝐂𝐚𝐭(_k)$). A DG category is a category enriched over $C^{dg}(k)$.
{% enddef %}
...
{% def %}
($\mathcal{FUN}$). Let $\mathcal{A}$ and $\mathcal{B}$ be DG categories, then $\mathcal{FUN}(\mathcal{A}, \mathcal{B})$ is a DG category. An object $F ∈ \mathcal{FUN}(\mathcal{A}, \mathcal{B})$ is called a DG functor, which is compatible with identity and compositions and satisfies that

$$\begin{equation}
F_{X,Y} ∈ \mathcal{HOM}_k ((X, Y)_\mathcal{A}, (FX,FY)_\mathcal{B}).
\end{equation}$$

For $F,G ∈ \mathcal{FUN}(\mathcal{A}, \mathcal{B})$, we define $\mathcal{HOM}(F,G) ∈ C^{dg}(k)$ as the complex of graded morphisms. A degree $n$ natural transformation $α$ is a collection of degree $n$ natural transformation $α_X ∈ (FX, GX)_\mathcal{B}^n$ ($∀ X ∈ \mathcal{A}$). The differential is computed elementwise in $\mathcal{B}$.
{% enddef %}
...
* $𝐅𝐮𝐧(F,G)$ is a category, as $\mathcal{FUN}(F,G)$ is a DG-catetory;
* An morphism in $𝐅𝐮𝐧(F,G)$ is labelled by $𝖮𝖻 (\mathcal{A})$ and the computation is done in $\mathcal{B}$; an morphism in $\mathcal{FUN}(F,G)$ is also labelled by $𝖮𝖻 (\mathcal{A})$ and the computation is done in $\mathcal{B}$.

{% thm %}
$\mathcal{FUN}$ is the internal Hom of $𝐃𝐆𝐂𝐚𝐭$, i.e. there is a natural isomorphism of DG categories

$$\begin{equation}
\mathcal{FUN}(\mathcal{A} ⊠ \mathcal{B}, \mathcal{C}) ≃ \mathcal{FUN}(\mathcal{A}, \mathcal{FUN}(\mathcal{B}, \mathcal{C})).
\end{equation}$$

{% pf %}
We show the correspondence on objects (= DG functors). A DG functor $F : \mathcal{A} ⊠ \mathcal{B} → \mathcal{C}$ induces a collection of $C^{dg}(k)$ morphisms

$$\begin{equation}
F_{(A,B),(A',B')} : \mathcal{HOM}_\mathcal{A}(A,A') ⊗ \mathcal{HOM}_\mathcal{B}(B,B') → \mathcal{HOM}_\mathcal{C}(F(A,B), F(A',B')).
\end{equation}$$

By closed monoidal structure of $C^{dg}(k)$, we have a collection of morphisms

$$\begin{equation}
G_{A,A'} (-) : \mathcal{HOM}_\mathcal{A}(A,A') → \mathcal{HOM}(\mathcal{HOM}_\mathcal{B}(B,B'), \mathcal{HOM}_\mathcal{C}(F(A,B), F(A',B'))).
\end{equation}$$

We show $G$ is a well-defined DG functor
