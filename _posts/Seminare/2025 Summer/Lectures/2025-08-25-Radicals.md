...
- it lifts idempotents from quotient rings;
...
- It suffices to show $φ _x$ lies in every maximal right ideal. If not, then there is $(φ _x) + 𝔪 = R$. We take $φ _x + m = 1$. Since $\operatorname{im} (φ _x) + \operatorname{im} (m) = P$, we see $m$ is surjective and thus has a right inverse. A contradiction!

Hence $(1 - φ _x)$ is a unit and $(1 - φ _x)(x) = 0$. We see $x = 0$.
\\
...
\\
Consider $φ =(1-r) : (x_0,x_1,\ldots) ↦ (x_0, x_1 - x_0 a_0, x_2 - x_1a_1, \ldots)$. When it is split monic, we denote the right inverse by
...
We see the ideal stablise. The rest is clear.
...
{% pf %}
(2 → 1). $\frac{R}{J(R)}$ satisfies descending chain condition of principal left ideals, hence semi-simple. We show $J(R)$ is transfinite nilpotent
