...
{% pf %}
Set $J = I ∖ \{0\}$. Let $π : M ⊕ L ⊕ X_0 ↠ M ⊕ L$ be canonical projection. Then $π : ∐_J X_∙ → M ⊕ L$ is an isomorphism, thus $π$ preserves and reflect the summands. Let $P_i$ be summands of $π (X_i)$ s.t.

$$\begin{equation}
M ⊕ L = M ⊕ ∐ _J P_i = ∐ _J π (X_∙ ).
\end{equation}$$

We take $Y_i := X_i ∩ π^{-1} (P_i)$ for $i ∈ J$. Note that $π : Y_i ≃ P_i$.
...
{% pf %}
Let $M$ and $M'$ be $|I|$-exchangable. Suppose $\text{all} = M ⊕ M' ⊕ N = ∐ _I X_∙$, then there are summands $Y_∙$ s.t. $\text{all} = M ⊕ ∐ _I Y_∙ = M ⊕ M' ⊕ N$. We apply $M' ⊕ (N ⊕ M) = ∐ _{I∪ \{∗\}} Y_∙$ where $Y_∙ = M$ is a summand of $N ⊕ M$. By the above proposition, we have $M' ⊕ N ⊕ M = M' ⊕ (∐ _I Z_∙ ) ⊕ M$.
{% endpf %}
...
{% thm %}
(Krull-Schmidt-Remak-Azumaya). Let $M$ be a coproducts of endolocal modules. Then any indecomposable decomposition of $M$ is unique up to permutation and isomorphism.

{% pf %}
Such decomposition exists by definition. We show any indecomposable summand is endolocal.

- Let $L$ be summand of $∐ \text{endolocal}$. By the same analysis of (2 → 3) above, $L$ is isomorphic to some summand.
...
$$\begin{equation}
\text{For Noetherian rings, Nil = Nilpotent!}
\end{equation}$$
